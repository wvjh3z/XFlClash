...
> **「点击后需先拉数据再跳转」统一模式**（§11 系统化修复）：不在当前页 `await` + 弹遮罩，
> 而是**立即 `xbPush` 到目标页**，由目标页用 `XbAsyncView` 自己加载转圈。续费走
> `PlanRenewLoader`（拉套餐 → 锁定当前套餐 → 渲染 `PlanDetailPage`），与「购买/更改」「流量重置」
> 口径完全一致（都是立即跳转 + 目标页转圈）。这样既统一交互，又天然免疫连点（叠加 §6.4 节流）。

### 6.5 后端富文本 HTML 渲染（套餐/教程正文，flutter_html）

后端知识库/套餐正文是「网页 HTML」，flutter_html 渲染有多处坑，统一沉成框架（§11）：
...
4. **点击后需先拉数据再跳转**：**立即 `xbPush` 到目标页**，由目标页用 `XbAsyncView` 自己加载转圈（如续费 `PlanRenewLoader`），与「购买/更改/流量重置」口径一致。不要在当前页 `await` + 弹遮罩，也不要在按钮里自己写 `_busy` + 转圈。
5. **页面跳转**：一律 `xbPush`（已内置 500ms 防连点节流），不裸用 `Navigator.push`。
6. **验证码冷却**：混入 `XbCooldownGuard`，别手写 Timer。
7. **颜色/圆角/间距**：一律用 `XbTokens`，不写裸数字裸色值。
8. **图标方块**：用 `XbIconBadge`，不手写 Container+BoxDecoration+Icon。
9. **金额/日期/流量**：用 `util/format.dart`，不在页面里 `toStringAsFixed`。
10. **弹窗**：说明类用 `XbInfoSheet`；确认类用 `xbConfirm`；认证类用 `XbSheetScaffold`。
11. **验证**：改完 `flutter analyze lib/xboard` → 视觉变更跑 golden（`--update-goldens`）→ 三守卫。
